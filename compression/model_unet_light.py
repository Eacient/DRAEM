...
class DepthSeperatableConv(nn.Moudle):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1):
        self.dw_conv = nn.Conv2d(in_channels, in_channels, kernel_size=3, groups=in_channels)
        self.bn = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace=True)
        self.pw_conv = nn.Conv2d(in_channels, out_channels, kernel_size=1)
    def forward(self, x):
        return self.pw_conv(self.relu1(self.bn1(self.dw_conv(x))))

class EncoderDiscriminative(nn.Module):
    def __init__(self, in_channels, base_width):
        super(EncoderDiscriminative, self).__init__()
        self.block1 = nn.Sequential(
            DepthSeperatableConv(in_channels, base_width),
            nn.BatchNorm2d(base_width),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width, base_width),
            nn.BatchNorm2d(base_width*2),
            nn.ReLU(inplace=True))
        self.mp1 = nn.Sequential(nn.MaxPool2d(2))
        self.block2 = nn.Sequential(
            DepthSeperatableConv(base_width,base_width*2),
            nn.BatchNorm2d(base_width*2),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width*2, base_width*2),
            nn.BatchNorm2d(base_width*2),
            nn.ReLU(inplace=True))
        self.mp2 = nn.Sequential(nn.MaxPool2d(2))
        self.block3 = nn.Sequential(
            DepthSeperatableConv(base_width*2,base_width*4),
            nn.BatchNorm2d(base_width*4),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width*4, base_width*4),
            nn.BatchNorm2d(base_width*4),
            nn.ReLU(inplace=True))
        self.mp3 = nn.Sequential(nn.MaxPool2d(2))
        self.block4 = nn.Sequential(
            DepthSeperatableConv(base_width*4,base_width*8),
            nn.BatchNorm2d(base_width*8),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width*8, base_width*8),
            nn.BatchNorm2d(base_width*8),
            nn.ReLU(inplace=True))
...
class DecoderDiscriminative(nn.Module):
    def __init__(self, base_width, out_channels=1):
        ...
        self.db_b = nn.Sequential(
            DepthSeperatableConv(base_width*(8+8), base_width*8),
            nn.BatchNorm2d(base_width*8),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width * 8, base_width * 8),
            nn.BatchNorm2d(base_width * 8),
        ...
        self.up1 = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                                 DepthSeperatableConv(base_width * 8, base_width * 4),
                                 nn.BatchNorm2d(base_width * 4),
                                 nn.ReLU(inplace=True))
        self.db1 = nn.Sequential(
            DepthSeperatableConv(base_width*(4+8), base_width*4),
            nn.BatchNorm2d(base_width*4),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width * 4, base_width * 4),
            nn.BatchNorm2d(base_width * 4),
        ...
        self.up2 = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                                 DepthSeperatableConv(base_width * 4, base_width * 2, kernel_size=3, padding=1),
                                 nn.BatchNorm2d(base_width * 2),
                                 nn.ReLU(inplace=True))
        self.db2 = nn.Sequential(
            DepthSeperatableConv(base_width*(2+4), base_width*2, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width*2),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width * 2, base_width * 2, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width * 2),
            nn.ReLU(inplace=True)
        )

        self.up3 = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                                 DepthSeperatableConv(base_width * 2, base_width, kernel_size=3, padding=1),
                                 nn.BatchNorm2d(base_width),
                                 nn.ReLU(inplace=True))
        self.db3 = nn.Sequential(
            DepthSeperatableConv(base_width*(2+1), base_width, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width, base_width, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width),
            nn.ReLU(inplace=True)
        )
        ...
        self.fin_out = nn.Sequential(DepthSeperatableConv(base_width, out_channels, kernel_size=3, padding=1))
...
class EncoderReconstructive(nn.Module):
    def __init__(self, in_channels, base_width):
        ...
        self.block1 = nn.Sequential(
            DepthSeperatableConv(in_channels,base_width, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width, base_width, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width),
        ...
        self.mp1 = nn.Sequential(nn.MaxPool2d(2))
        self.block2 = nn.Sequential(
            DepthSeperatableConv(base_width,base_width*2, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width*2),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width*2, base_width*2, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width*2),
        ...
        self.mp2 = nn.Sequential(nn.MaxPool2d(2))
        self.block3 = nn.Sequential(
            DepthSeperatableConv(base_width*2,base_width*4, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width*4),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width*4, base_width*4, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width*4),
            nn.ReLU(inplace=True))
        self.mp3 = nn.Sequential(nn.MaxPool2d(2))
        self.block4 = nn.Sequential(
            DepthSeperatableConv(base_width*4,base_width*8, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width*8),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width*8, base_width*8, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width*8),
            nn.ReLU(inplace=True))
        self.mp4 = nn.Sequential(nn.MaxPool2d(2))
        self.block5 = nn.Sequential(
...
class DecoderReconstructive(nn.Module):
    def __init__(self, base_width, out_channels=1):
        ...
        self.db1 = nn.Sequential(
            ...
            nn.BatchNorm2d(base_width*8),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width * 8, base_width * 4, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width * 4),
            nn.ReLU(inplace=True)
        )
        ...
        self.db3 = nn.Sequential(
            ...
            nn.BatchNorm2d(base_width*2),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width*2, base_width*1, kernel_size=3, padding=1),
        ...
        self.up4 = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                                 ...
                                 nn.ReLU(inplace=True))
        self.db4 = nn.Sequential(
            DepthSeperatableConv(base_width*1, base_width, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width),
            nn.ReLU(inplace=True),
            DepthSeperatableConv(base_width, base_width, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width),
        ...
        self.fin_out = nn.Sequential(DepthSeperatableConv(base_width, out_channels, kernel_size=3, padding=1))
        #self.fin_out = DepthSeperatableConv(base_width, out_channels, kernel_size=3, padding=1)
